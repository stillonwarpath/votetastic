...
# d) Copy project files into the docker image and build the app
COPY . /app
...
# b) From 'builder' copy the site to default nginx public folder
COPY --from=builder /app/dist/voting-app /usr/share/nginx/html

# c) copy our own default nginx configuration to the conf folder
